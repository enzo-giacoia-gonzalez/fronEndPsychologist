...
const handleHiddenValue = (e:any) => {
  ...
  return (
    ...
    <Input onChange={(e)=>{handleTitle(e)}} placeholder='Titulo' value={postInput.titulo===undefined?receiptById?.titulo:postInput.titulo} sx={{border:1, marginBottom:1, borderRadius:"4px"}} name='titulo' type='text'></Input>
      <Input onChange={(e)=>{handleTime(e)}} placeholder='Hora y dia de la sesion' value={postInput.fechayhora===undefined?receiptById?.fechayhora:postInput.fechayhora} sx={{ border:1, marginBottom:1, borderRadius:"4px"}} name='fechayhora' type='datetime-local'></Input>
      <Input onChange={(e)=>{handlePrecio(e)}}  placeholder='Precio de la sesion' value={postInput.precio===undefined?receiptById?.precio:postInput.precio}  sx={{border:1, marginBottom:1, borderRadius:"4px"}} name='precio' type='text'></Input>
      <Input readOnly onChange={(e)=>{handlePago(e)}}  placeholder='pago' value={postInput.pago===undefined?receiptById?.pago:postInput.pago}  sx={{border:1, marginBottom:1, borderRadius:"4px"}} name='precio' type='text'></Input>
      <Input readOnly placeholder='Nombre del usuario' value={postInput.usuario===undefined?userById?.nombre:userByMail.nombre} sx={{border:1, marginBottom:1, borderRadius:"4px"}} type='text'></Input>
      <Input onChange={(e)=>{handleHiddenValue(e)}} type="hidden" value={postInput.idUsuario===undefined?userById?.uid:userByMail.uid}  />
     

  
        
      <Button onClick={()=>{putReceipt(postInput.titulo, postInput.fechayhora,postInput.precio, postInput.pago, postInput.idUsuario,receiptById?._id)}} sx={{borderRadius:"4px", border:1, marginBottom:1 ,bgcolor:'#BAA0C8', color:'black', ":hover":{bgcolor:'#6C2273', color:'white'}}}>Agregar servicio</Button>
    </Card>
    </Grid>
  )
}
